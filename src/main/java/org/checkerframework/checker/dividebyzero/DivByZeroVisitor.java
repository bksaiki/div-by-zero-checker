...
import org.checkerframework.checker.dividebyzero.qual.AlwaysZero;
import org.checkerframework.checker.dividebyzero.qual.MaybeZero;
...
public class DivByZeroVisitor extends BaseTypeVisitor<DivByZeroAnnotatedTypeFactory> {
  ...
  private boolean errorAt(BinaryTree node) {
    ...
    if (DIVISION_OPERATORS.contains(node.getKind())) {
      // The division operator is represented by the DIVIDE kind.
      if (hasAnnotation(node.getRightOperand(), AlwaysZero.class)
          || hasAnnotation(node.getRightOperand(), MaybeZero.class)) {
        return true;
      }
    }
  ...
  private boolean errorAt(CompoundAssignmentTree node) {
    ...
    if (DIVISION_OPERATORS.contains(node.getKind())) {
      // The division operator is represented by the DIVIDE kind.
      if (hasAnnotation(node.getExpression(), AlwaysZero.class)
          || hasAnnotation(node.getExpression(), MaybeZero.class)) {
        return true;
      }
    }
